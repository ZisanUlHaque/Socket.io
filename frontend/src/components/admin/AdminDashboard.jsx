...
const AdminDashboard = ({ socket, connected, onShowNotification, onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (!savedPass) {
      onShowNotification("Admin session expired, please login again", "error");
      onLogout();
      return;
    }

    socket.emit("adminLogin", { password: savedPass }, (res) => {
      if (res.success) {
        loadOrders();
        loadStats();
      } else {
  ...
  const loadOrders = () => {
    ...
    socket.emit("getAllOrders", {}, (response) => {
      ...
      if (response.success) {
        ...
        onShowNotification(response.message || "Failed to load orders", "error");
      }
    });
  };
  ...
  const handleAcceptOrder = (order) => {
    ...
    socket.emit("acceptOrder", { orderId: order.orderId, estimatedTime }, (response) => {
      if (response.success) {
        ...
        onShowNotification(response.message || "Failed to accept order", "error");
      }
    });
  };
  ...
  const handleRejectOrder = (order) => {
    ...
    const index = parseInt(reasonIndex, 10) - 1;
    ...
    socket.emit("rejectOrder", { orderId: order.orderId, reason }, (response) => {
      if (response.success) {
        ...
        onShowNotification(response.message || "Failed to reject order", "error");
      }
    });
  };
  ...
  const handleUpdateStatus = (orderId, newStatus) => {
    socket.emit("updateOrderStatus", { orderId, newStatus }, (response) => {
      if (response.success) {
        ...
        onShowNotification(response.message || "Failed to update status", "error");
      }
    });
  };
  ...
  const tabs = [
    ...
    },
    {
      ...
      inactiveClass: "text-slate-600 hover:bg-purple-50 hover:text-purple-700",
      badgeActive: "bg-white/20 text-white",
      badgeInactive: "bg-slate-100 text-slate-600",
    },
    {
      ...
      inactiveClass: "text-slate-600 hover:bg-emerald-50 hover:text-emerald-700",
      badgeActive: "bg-white/20 text-white",
      badgeInactive: "bg-slate-100 text-slate-600",
    },
    {
      ...
      inactiveClass: "text-slate-600 hover:bg-rose-50 hover:text-rose-700",
      badgeActive: "bg-white/20 text-white",
      badgeInactive: "bg-slate-100 text-slate-600",
    },
  ];
  ...
  const StatCard = ({ label, value, color, icon }) => (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100/50 hover:shadow-lg hover:-translate-y-[2px] transition-all duration-300 group">
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 pb-20">
      {/* Header */}
      <header
        className={`sticky top-0 z-40 border-b border-slate-200/60 backdrop-blur ${
          connected
        ...
        {stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10">
            ...
            <StatCard
              ...
              color="text-black bg-emerald-500"
              icon="✓"
            />
          </div>
        )}
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <div className="bg-white rounded-3xl border border-dashed border-slate-300 p-12 text-center">
                ...
                <p className="text-slate-500">No orders in this category right now.</p>
              </div>
            ) : (
